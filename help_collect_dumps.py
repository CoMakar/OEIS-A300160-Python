...
def main():
    ...
    chdir("dumps")
    files = listdir()
    files = filter(lambda f: f.endswith("dump.json"), files)
    ...
    for file in files:
        with open(file, 'r') as dump_file:
            log(f"{dump_file.name:<25} file found")
            ...
            try:
                ...
                if numbers is not None and len(numbers) != 0:
                    log(f"{dump_file.name:<25} {len(numbers)} numbers collected")
                    valid_numbers.extend(numbers)
                else:
                    log(f"{dump_file.name:<25} numbers not found")
                
            except json.JSONDecodeError as e:
                log(f"{dump_file.name:<25} contains invalid JSON: {e}")
